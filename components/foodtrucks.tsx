...
const FoodTrucks: React.FC = () => {
  ...
  const [tab, setTab] = useState<'image' | 'structured'>('structured');
  ...
  // Auto-trigger OCR once when landing on the Structured tab
  useEffect(()=>{
    ...
    // Defer a tick to ensure button mounts
    const id = setTimeout(()=>{
      if (!autoRunRef.current && ocrBtnRef.current && !ocrBtnRef.current.disabled) {
        autoRunRef.current = true;
        ocrBtnRef.current.click();
      }
    }, 0);
    return ()=> clearTimeout(id);
  }, [tab, tableOcrLoading, tableOcrResult, active]);
  ...
  return (
    <div className="relative snap-center shrink-0 w-[300px] rounded-lg max-w-[300px] px-5 py-3 flex flex-col bg-content1 border-1 border-foreground/10">
      <h1 className="mb-2 text-2xl text-primary/90 font-extrabold flex items-center gap-2">Food Trucks <span className="text-[10px] uppercase tracking-wide px-1 py-0.5 rounded bg-yellow-500/20 text-yellow-600 dark:text-yellow-400">beta</span></h1>
      {/* Tabs */}
      <div className="flex text-[11px] mb-2 rounded overflow-hidden border border-foreground/10 w-full">
        {['structured', 'image'].map(t=> (
          <button key={t} onClick={()=>setTab(t as any)} className={`flex-1 py-1 capitalize ${tab===t? 'bg-primary/20 text-primary font-semibold':'bg-content2/40 text-foreground/50'}`}>{t}</button>
        ))}
      </div>
      ...
      {tab==='structured' && (
        <div className="flex flex-col gap-2 text-xs">
          ...
          {usingOcrTable && <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium">Using OCR table data (not saved)</p>}
          <div className="flex flex-col gap-1 pr-1">
            ...
            {usingOcrTable && (
              <p className="text-[9px] text-foreground/40 italic mt-1">Full schedule derived from table OCR.</p>
            )}
          </div>
          <div className="mt-2">
            {/* Table OCR pipeline */}
            <button
              ref={ocrBtnRef}
              disabled={tableOcrLoading || !active?.url}
              onClick={async()=>{
                if(!active?.url) return;
              ...
              className="w-full text-[11px] mt-2 px-2 py-1 rounded bg-blue-500/20 hover:bg-blue-500/30 text-blue-600 dark:text-blue-400 transition disabled:opacity-40">{tableOcrLoading? `Table OCR ${(tableOcrProgress*100).toFixed(0)}%`:'Run Table OCR (new)'}
            </button>
            {tableOcrError && <p className="text-[10px] text-red-500 mt-1">Table OCR error: {tableOcrError}</p>}
            {tableOcrResult && (
              <div className="mt-2 border border-foreground/10 rounded p-1 bg-content2/50">
                <p className="font-semibold text-[10px] mb-1 flex items-center gap-2">Table OCR Result <span className="text-foreground/40 font-normal">{tableOcrResult.length} rows</span></p>
                <table className="w-full table-fixed border-collapse text-[9px]">
                  <thead>
                    <tr className="bg-foreground/10">
                      {Object.keys(tableOcrResult[0]||{}).map(h=> <th key={h} className="px-1 py-0.5 text-left font-semibold sticky top-0 bg-foreground/10 backdrop-blur z-10">{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {tableOcrResult.slice(0,50).map((r,i)=>(
                      <tr key={i} className="odd:bg-foreground/5">
                        {Object.keys(tableOcrResult[0]).map(h=> <td key={h} className="px-1 py-0.5 align-top">{r[h]}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <p className="text-[9px] text-foreground/40 mt-1">OCR runs locally in your browser; no server upload.</p>
          </div>
        </div>
      )}
      {tab==='image' && (
      ...
      <div className="mt-3 p-2 rounded bg-foreground/5 text-[10px] leading-snug text-foreground/50">
        ...
        <p>Images auto-fetched (hourly cache). Structured view is a manual beta dataset; OCR + crowdsourced edits planned.</p>
      </div>
    </div>
  );
};
